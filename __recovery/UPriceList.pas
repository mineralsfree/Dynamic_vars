 ...
 type
  ...
  implementation
   ...
   begin
     ...
     temp.INF.productCount:=StrToInt(InputBox('EnterCount','Количество','15'));
...
procedure writePriceList (const head:PriceListADR; Grid:TStringGrid);
    ...
    Grid.Cells[3,Grid.RowCount-1]:=IntToStr(temp.INF.productCount);
  ...
  begin
    ...
    function GetCount(const head:PriceListADR; name:string):Integer;
    var temp:PriceListADR;
    begin
      temp:=ObjAdrOfname(head,name);
      result:=temp^.INF.productCount;
    end;
